...
abstract class ProductCategoryDataSource {
  ...
  Future<ProductCategoryModel> ProductCategory(NoParams params);
}

class ProductCategoryDataSourceImpl implements ProductCategoryDataSource {
  @override
  Future<ProductCategoryModel> ProductCategory(NoParams params) async {
    ...
    try {
      http.Response response = await http.get(Uri.parse("http://95.216.141.221/api/v1/setting/dropdown-category/"), headers: header);
      return ProductCategoryModel.fromJson(jsonDecode(utf8.decode(response.bodyBytes)));
    } on DioError catch (e) {
